...
MODEL_TRAINING_STEPS = ["ASPECT_CLASSIFICATION", "POLARITY_CLASSIFICATION", "ASPECT_EXTRACTION"]
TRAIN_FILE_NAME = 'restaurant_train.xml'
TEST_FILE_NAME = 'restaurant_test.xml'
VALIDATION_FILE_NAME = 'restaurant_validation.xml'

def main():
    # Reads Data
    train_reviews = get_reviews(os.path.join("input_files", TRAIN_FILE_NAME))
    validation_reviews = get_reviews(os.path.join("input_files", VALIDATION_FILE_NAME))
    train_reviews.extend(validation_reviews)
    test_reviews = get_reviews(os.path.join("input_files", TEST_FILE_NAME))

    for TASK in MODEL_TRAINING_STEPS:
        if TASK == "ASPECT_CLASSIFICATION":
            ...
            model = aspect_classification(train_reviews=train_reviews,
                                          ...
                                          output_path=os.path.join("models", "aspect_classification_model"))
            ...
            model = polarity_detection(train_reviews=train_reviews,
                                       ...
                                       output_path=os.path.join("models", "polarity_detection_model"))
            ...
            model = aspect_extraction(train_reviews=train_reviews,
                                      test_reviews=test_reviews,
                                      tags=tags,
                                      output_path=os.path.join("models", "aspect_extraction_model"))
